    ...
    class Labfunk
    {
        ...
        public void Colors(PictureBox panel, int _rows, int _columns,int lvl, bool wait)
        {
            ...
            else if (_grid[_columns, _rows, 0] == Schi && panel.BackColor != Color.Orange)
            {
                ...
                else if (lvl == 10 && _rows > 10 && _rows < 21)
                {
                    panel.BackgroundImage = Boden;
                    panel.BackColor = Color.LightGray;
                }
        ...
        public string Person(Panel panMain, PictureBox panel, int _row, int _column, int Level)
        {
            ...
            #endregion
            #region Level 10
            if (_grid[_column, _row, 1] == Player && _grid[_column, _row, 0] == Pers && Level == 10)
            {
                if (_row == 2 && _column == 5 && count == 1)
                {

                    Dialog = "Ich finde aus irgend einen Grund immer wieder Goldmünzen im Fluss, wo kommen die her ";

                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 6 && _column == 7 && count == 1)
                {
                    Dialog = "Ich habe gehört das Tiere im Elfenreich gar nicht feindselig sind in Peskar, ich frage mich warum.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 8 && _column == 12 && count == 1)
                {
                    Dialog = "Wir haben nicht viele besucher in unserem Dorf";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 7 && _column == 15 && count == 1)
                {
                    Dialog = "Bemalra ist die Grenzstadt zu Peskar, man kann dort viele Elfen und Zwerge treffen. Die Zwerge interessieren sich stark an dem Gebirge ";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 7 && _column == 27 && count == 1)
                {
                    Dialog = "Elfen mögen Fleich nicht besonders, aber Fisch und unser Brot. Das Importieren sie in Massen, dafür bekommen wir Schmackhafte Früchte und Magische Stoffe";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 6 && _column == 29 && count == 1 || _row == 4 && _column == 29 && count == 1)
                {
                    Dialog = "Willkommen in Bemalra, wir wüschen euch einen Schönen aufenthalt.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 2 && _column == 32 && count == 1)
                {
                    Dialog = "In Vorlondin versammelteten sich die Helden und stellten mit den Reiche eine Armee auf die das Empire besiegte.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 7 && _column == 32 && count == 1)
                {
                    Dialog = "Ich bin Teil der Orklas Boschaftertrupps. Wir pflegen gute Beziehungen mit Vorlondin.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 1 && _column == 35 && count == 1)
                {
                    Dialog = "Elfische Stoffe sind so schön anzuschauen, und die sind so leicht aber wiederstands fähig.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 3 && _column == 38 && count == 1)
                {
                    Dialog = "Preise die Erde, die Gemeinschaft, die Freundschaft und das Leben. Alles ist Teil dieser Welt.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 9 && _column == 35 && count == 1)
                {
                    Dialog = "Die Zwerge wollen nicht das wir im den Bergen nach Erzen suchen, wieso nicht?";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 10 && _column == 38 && count == 1)
                {
                    Dialog = "Menschen habe ein Talent fürs kochen, das muss ich zugeben. Ihr Brot ist wirklich gut.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 15 && _column == 35 && count == 1)
                {
                ...
                else if (_row == 12 && _column == 37 && count == 1)
                {
                ...
                else if (_row == 14 && _column == 37 && count == 1)
                {
                ...
                else if (_row == 19 && _column == 36 && count == 1)
                {
                    Dialog = "Bomben, Brauchst du Bomben, je 1000 Gold pro Stück. Aber nur wenn du weniger als 3 Stück bei dir hast, sonst kann ich sie wegen sicherheits Gründen nicht geben";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 18 && _column == 30 && count == 1)
                {
                    Dialog = "Hey Menschlein, ich werd dich nicht dran hindern hier weiterzugehen. Aber ich warne dich die Höhlen sind sehr gefährlich, und nein ich meine nicht die Goblins. Die könnte ein Kind besiegen.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 29 && _column == 25 && count == 1)
                {
                    Dialog = "Hallo, du kannnssst hieeeerr Raassstenn. Ich ssccchhaauee dass dir nnichts paaasssiert. Nurrr füür 300 Goooolld";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 23 && _column == 38 && count == 1)
                {
                    Dialog = "Obwohl wir Vorlondin mögen, so werden die Elfen niemals vergessen was uns in den frühereren Zeiten angetan wurden. Soviele sind gestorben.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 36 && _column == 13 && count == 1 || _row == 32 && _column == 29 && count == 1)
                {
                    Dialog = "Mensch, falls du den Wald betrittst merke dir folgendes. Alle Tiere, egal welches ist nicht gestattet gejagd zu werden. Aber du must auch keine Angst haben sie werden die auch nicht angreiffen.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 34 && _column == 37 && count == 1 || _row == 38 && _column == 22 && count == 1)
                {
                    Dialog = "Heul heul Woof Woof(Wer bist du?)";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 29 && _column == 24 && count == 1 )
                {
                    Dialog = "Alles in unserem Land ist gesegtnet, deshalb schmecken unsere Früchte auch so köstlich";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 26 && _column == 21 && count == 1)
                {
                    Dialog = "Wir Elfen ernähren uns Hauptsächlich von Früchten, Gemüse und Brot der Menschen. Manchmal essen wir auch Fisch.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 12 && _column == 24 && count == 1)
                {
                    Dialog = "Ah, es ist so Friedlich hier, man könnte meinen wir befänden uns in einer anderen Welt.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 23 && _column == 3 && count == 1)
                {
                    Dialog = "Menschen lieben den Apfelwein denn wir Produzieren";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 35 && _column == 11 && count == 1 || _row == 33 && _column == 11 && count == 1)
                {
                    Dialog = "Willkomen Mensch in Eichentann. Letztes der Elfenreiche auf Ressos";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 37 && _column == 9 && count == 1)
                {
                    Dialog = "Unsere Herrin sorgt dafür, dass in unserem Land Frieden eingehalten wird.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 33 && _column == 7 && count == 1)
                {
                    Dialog = "Mensch für 3000 Gold kannst du diesen Mana Brunnen verwenden. Dein Mana ist nacher wieder Voll.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 35 && _column == 6 && count == 1)
                {
                    Dialog = "Fast von jedem Land bekommen wir besucher, nur das Empire und der Orden besuchen uns sein 100 Jahren nicht mehr.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 33 && _column == 4 && count == 1)
                {
                    Dialog = "Mensch, du kannst dich zu den wenigen Menschen zählen die ausserhalb von Trenten einen Runenmagier sieht. ";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 37 && _column == 4 && count == 1)
                {
                    Dialog = "Elfenmagie ist komplet anders aufgebaut zu unserer Magie in Dravos, aber auch komplett anders zu Runenmagie der Zwerge";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 36 && _column == 2 && count == 1 || _row == 34 && _column == 2 && count == 1)
                {
                    Dialog = "Unsere Herrin weiss von dir, sie erwartet dich. Bitte tritt ein";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                Kommen = Dialog;
            }
        ...
        public string Kommentar(Panel panMain, PictureBox panel, int _row, int _column, int Level, string Dialog)
        {
            ...
            #endregion
            #region Level 10
            if (_grid[_column, _row, 1] == Player && _grid[_column, _row, 0] == Kome && Level == 10)
            {
                if (_row == 3 && _column == 3 && count == 1)
                {

                    Kommen = "Wenn ich weiter der Strasse folge, dann sollte ich die Stadt Bemalra erreichen. Dort befindet sich auch der durchgang nach Peskar.";

                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 22 && _column == 30 && count == 1)
                {
                    Kommen = "Südwestlich von hier sollte Eichentann stehen, bald werde ich wissen wer ich bin. ";

                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 34 && _column == 11 && count == 1)
                {
                    Kommen = "Endlich Eichentan nicht mehr lange, dann weiss ich wieso der Baron mich loss werden wollte(spoiler du wirst es nicht mehr erfahren)";

                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 18 && _column == 18 && count == 1)
                {
                    Kommen = "Hm ein kleiner Raum mit einem Feuer in der Ecke, ein Luftzug? Aber von wo?";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 16 && _column == 16 && count == 1)
                {
                    Kommen = "Uf bin ich erschöpft, wenn ich jetzt schon auf solche Monster treffe, was kommt er recht später.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 13 && _column == 13 && count == 1)
                {
                    Kommen = "Erst der Fette und nun dieses Phantom, was kommt als nächstes";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 13 && _column == 10 && count == 1)
                {
                    Kommen = "Ich glaubes es kaum das ich all das besiegt habe. Wow ist das heiss hier, ist das Lava?";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 13 && _column == 5 && count == 1)
                {
                    Kommen = "Schlimmer kanns ja nicht mehr werden.";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                else if (_row == 14 && _column == 2 && count == 1)
                {
                    Kommen = "Ja klar, ein Spinner der denkt er sei Gott. Ich hoffe wenigstens, dass es sich gelohnt hat, durch diese ganzen Höhlen zu gehen. ";
                    _grid[_column, _row, 2] = empty.ToString();
                }
                Dialog = Kommen;
            }
        ...
        public bool Fight(Panel panMain, PictureBox panel, int _row, int _column,bool fight,int Armor ,int Schleif,int encoun, int Bombe, int Mana,int Lvl,int tresure)
        {
            ...
            if (_grid[_column, _row, 1] == Player && _grid[_column, _row, 4] == Bos3 && panel.Image == Secret && count == 1 && encoun == 17)
            {
                ...
                else if (Points >= -8 && Points<=-3)
                {
                    ...
                    {//no Bombe or Magic
                        MonsterResult = 5;
                        MessageBox.Show("Verletzt aber noch nicht Tod, versucht du den schlägen des Troll auszuweichen. All dies ist sehr schmerzhaft aber jedes mal wenn er seine Axt in den Boden jagd, schlägt du mehre male auf seine Arme und seine Beine. Als der Troll merkt das er seine Axt nicht mehr heben kann, gerät er in panik und rennt davon. Danach fallst du zu Boden und bist froh das der Kampf vorüber ist.");
                        fight = true;
                        _grid[_column, _row, 2] = empty.ToString();
                    }
                    ...
                    {
                        MonsterResult = 7;
                        MessageBox.Show("Ihr seid beide angeschlagen, er hat angst vor der Magie die du anwendest. Du hast Angst vor seinen Hieben mit der Axt. Doch stürmt ihr mehrmals auf einander zu und schlägt euch mehrmals, er traff dich 2 mal mit seiner nich Axt Hand und dafür mehreremale gegen seine Axthand. Er lässt vor schmerzen die Axt fallen und rennt davon, selber merkst du aber, dass du selbst keine Energie mehr hast. Du kippst um und wirst für Kurze Zeit ohmächtig.");
                        fight = true;
                        _grid[_column, _row, 2] = empty.ToString();
                    }
                    ...
                    {
                        MonsterResult = 9;
                        MessageBox.Show("Er hat deine Bomben satt. Er stürmt in deine Richtung. Du weichst ihn so gut wie es geht aus, aber der Luftzug seines Angriff wirft dich um. Gekonnt weichst du seinen Angriff aus in dem du dich wegrollst. Bis auf einmal seine Axt im Boden eingeklemmt scheint. Du merkst du musst jetzt handeln. Du greift seine Axt Hand an, der Troll schreit vor Schmerzen auf und rennt vor dir weg. Du setzt dich auf einen Stein und verarbeitest dieses erlebnis.");
                        fight = true;
                        _grid[_column, _row, 2] = empty.ToString();
                    }
                    ...
                    {
                        MonsterResult = 11;
                        MessageBox.Show("Dir schmerzt dein ganzer Körper, aber auch der Troll schnauf stark, keiner von euch beiden sieht ein gutes Ende dieses Kampfes. und dann passiert etwas, dass du komplett nicht erwartet hast. Der Troll geht einfach, er denkt sich dieser Mensch ist es die Mühe nicht Wert.");
                        fight = true;
                        _grid[_column, _row, 2] = empty.ToString();
                    }
                    ...
                    {
                        MonsterResult = 13;
                        MessageBox.Show("Der Troll, sehr angschlagen von der Brandwunde die er von dir erhalten, schaut dich mit scharfen Augen an. Du selber stark verletzt schaust in genau so stark an. Anscheinend möchte der Troll nicht weiterkämpfen, den er geht weg und lässt dich in Ruhe.");
                        fight = true;
                        _grid[_column, _row, 2] = empty.ToString();
                    }
                    else
                    {//no Bombe or Magic
                        MonsterResult = 2;
                        MessageBox.Show("Einige würden deinen Kampf mit dem Troll den Kampf des Jahrhunderts nennen. Jedoch wird nie jemand den Kampf sehen um draus eine Legende zu erstellen. Ihr seit beide mit unzähligen Wunden übersäht. Ihr beide stürmt nun auf euf zu und wollt euch den Finalen Schlag geben. Du schaftst es gerade noch seinen Hieb auszuweichen. Du schneides Tief in einer seiner schon vorhanden Wunden. Der Troll fällt um und ist gestorben(Glückwünsch dies ist das einzige Ende bei der der Spieler viel Leben verliert, aber auch den Troll tötet.)");
                    }
                }
            ...
            {//Drache
                ...
                {
                    MessageBox.Show("Er lässt dich durch.");
                    fight = true;
                        _grid[_column, _row, 2] = empty.ToString();
                    DragonResult=1;
                }
...
////////////////////////////////////////////////////
                    DialogResult dialogResult2 = MessageBox.Show("Der Drache greift dich an.Kämpfen(Fliehen ist wie immer Tod)", "Some Title", MessageBoxButtons.YesNo);
                    if (dialogResult2 == DialogResult.Yes)
                    {
                        MessageBox.Show("Ich muss ehrlich sein ich habe keine Idee wie ich diesen Kampf gestalten soll.(Daher brauchst du einfach 20 Rüstung 10 Schleifsteine und 5 Mana)");
                        if (Armor>=20&& Schleif>=10 && Mana>=5)
                        {
                            DragonResult = 2;
            ...
            {//Entwickler  
                MessageBox.Show("Eine Göttliche Präsenz erschein vor dir. Ich gratuliere dir das du meine 4 Bosse besiegt hast, nun bin aber ich dein Gegner, der Erschaffer dieser Welt. All diese Hinweise die du fandes. Die sind von mir. Ich gebe dir Fünf Fragen. Sollte ich mit deinen Antworten Zufrieden sein, dann hast du gewonnen, sonst komm einfach wieder fals du noch Respawns hast. Fals du keine mehr hasz, tja dann hattest du Pech");
                DialogResult dialogResult = MessageBox.Show("Frage Nummmer 1. Wie fandest du meine Kreation, alle Grafiken ausser dem Felsen habe ich persöhnlich erstellt. Ich habe mir viel mühe gegeben. Sie habe ihren Charme oder?", "Some Title", MessageBoxButtons.YesNo);
                if (dialogResult == DialogResult.Yes)
                {
                    Ques++;
                    MessageBox.Show(" Danke hat alles ziehmlich lange gebraucht");
                }
                else if (dialogResult == DialogResult.No)
                {
                    Ques--;
                ...
                DialogResult dialogResult2 = MessageBox.Show("Frage Nummmer 2. Soll ich die Welt erweitern oder vielleicht mit einem Spielentwicklungstool neu erstellen. Oder willst du das ich Aufhöre.", "Some Title", MessageBoxButtons.YesNo);
                if (dialogResult2 == DialogResult.Yes)
                {
                    Ques++;
                    MessageBox.Show(" Ich werds mir überlegen, aber danke für die netten Wort");
                }
                else if (dialogResult2 == DialogResult.No)
                {
                    Ques--;
                    MessageBox.Show(" Ich sehe ich sollte woll nicht mehr denken, dass ich ein Spiel programmiern kann");
                }
                DialogResult dialogResult3 = MessageBox.Show("Frage Nummmer 3. Bist du der Meinung, Gift sei das Beste Feature im Spiel.", "Some Title", MessageBoxButtons.YesNo);
                if (dialogResult3 == DialogResult.Yes)
                {
                    Ques--;
                    MessageBox.Show(" Nein es ist ein schlechtes Feature, ich gebe es zu. Aber ohne es wäre das Spiel einfach nur durchlaufen. ");
                }
                else if (dialogResult3 == DialogResult.No)
                {
                    Ques++;
                    MessageBox.Show(" Ich stimme es zu, aber es ist notwendig");
                }
                DialogResult dialogResult4 = MessageBox.Show("Frage Nummmer 4. Gefällt dir das Aussehen des Drachen, ja ich weiss ich habe keinen Kampfscene einprogrammiert. Ehrlich gesagt habe ich keine Ahnung wie ich einen Kampf einprogrammieren kann bei der du Gewinnen könntest. Aber gefällt dir wenigstens das Aussehen.", "Some Title", MessageBoxButtons.YesNo);
                if (dialogResult4 == DialogResult.Yes)
                {
                    Ques++;
                    MessageBox.Show(" Danke, ich habe etwa eine Stunde gebraucht ihn zu designen");
                }
                else if (dialogResult4 == DialogResult.No)
                {
                    Ques--;
                    MessageBox.Show(" Der Drache würde dich jetzt Fressen wenn er hier wäre");
                }
                DialogResult dialogResult5 = MessageBox.Show("Frage Nummmer 5. Hat du die Lore und Spielwelt die kreiert habe. Wenigsten ein Wenig spanned gefunden?(übrigens alle fragen gegen + oder - Punkt, nur wenn du -5 hast töte ich dich) ", "Some Title", MessageBoxButtons.YesNo);
                if (dialogResult5 == DialogResult.Yes)
                {
                    Ques++;
                    MessageBox.Show(" Leider konnte ich dir nur einen kleinen Teil dieser Welt zeigen. Aber wer weiss vielleicht eines Tages.");
                }
                else if (dialogResult5 == DialogResult.No)
                {
                    Ques--;
                    MessageBox.Show("Geh einfach, schliess das Spiel. Spiel Fortnight");
                }
                if (Ques==-5)
                {
                    EntwicklerR = 1;
                    MessageBox.Show(".......... Du hast extra alles falsch beantwortet oder. Fals nein wow wieso spielst du dieses Spiel überhaupt.");
                }
                else
                {
                    EntwicklerR = 2;
                    MessageBox.Show("Danke das du dieses Spiel gespielt hast. Ich habe sehr Lange gebraucht alles zu erstellen und muss erhlich sein ich brauche ne Pause. Aber es Freut mich falls dir das Spiel gefallen hat. Falls du unter 0 Punkte bist, dann frage ich mich wieso spielst du überhaupt. Trotzdem danke. Falls ich dich kenne man sieht sich sonst, viel glück in deinem Leben");
                }
            }
